...
; pad = the padding added to make the new row a multiple of 4
 (defun fmt-img (xs txs nrow pad)
  ...
  (defun steg-file-bytes (bmp-bytes)
    (let* ((hdr-img (hdr/img bmp-bytes))                      ;splits header from image data
           (hdr (car hdr-img))                                ;header
           (img (cadr hdr-img))                               ;image data
           (row (pix-W hdr))  
           (height (pix-H hdr))                                ;row width
           (fimg (magnify-tail img nil))
           (dbl (double (* 2 row) fimg nil))  					;rearrange pixels
           (pad (padding (* 2 row)))                                 ;determine number of bytes to add
           (rfimg (fmt-img dbl nil (* 2 row) pad))                  ;format rows to multiple of four
           (imgdata (sizing (len rfimg)))                     ;determine image data size (little-endian)
           (filedata (sizing (+ (len rfimg) (len hdr))))      ;determine file size (little-endian)
           (ref (reform hdr filedata (sizing (/ (* 2 row) 3)
                                             ) (sizing (/ (* 2 height) 3)
                                             ) imgdata))
           )               ;edit header for new image
      (append ref rfimg)))                                    ;put bitmap back together
    ...
    (mv-let (bmp err-opn-bmp state)
         ...
         (if err-opn-bmp
            ...
            (byte-list->binary-file
             (string-append "magnify-" bmp-fn)
             (steg-file-bytes bmp) state))))
