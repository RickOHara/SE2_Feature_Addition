...
(defproperty bw8-integrity-test :repeat 200
  (xs :value (random-list-of (random-between 0 255) :size 1024))
  (= (len (bw8 xs 250 nil)) (len xs)))

;Predicate based test for bw8
; Tests the property that the function changes pixel values to
; 0 or 255 only
(defproperty bw8-value-test :repeat 200
  (xs :value (random-list-of (random-between 0 255) :size 1024)
   x :value (random-between 0 255))
  (or (= (Nth x (bw8 xs 250 nil)) 0) (= (Nth x (bw8 xs 250 nil)) 255)))
  
;Predicate based test for blu-w
; Tests the property that the function returns the same number
; of bytes as the original image data.  It does not add to or 
; subtract pixels from the image
(defproperty blu-w-integrity-test :repeat 200
  (xs :value (random-list-of (random-between 0 255) :size 1024))
  (= (len (blu-w8 xs 250 nil)) (len xs)))

;Predicate based test for blu-w
; Tests the property that the function changes pixel blue channel
; values to 150 or 255 only
(defproperty blu-w-value-test :repeat 200
  (xs :value (random-list-of (random-between 0 255) :size 1024)
   x :value (random-between 0 255))
  (let* ((n (+(* x 4) 3)))
    (or (= (Nth n (blu-w8 xs 250 nil)) 150) (= (Nth n (blu-w8 xs 250 nil)) 255))))
